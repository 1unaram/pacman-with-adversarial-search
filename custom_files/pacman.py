...
from util import manhattanDist, nearestPoint
...
class ClassicGameRules:
    ...
    def win(self, state, game):
        if not self.quiet:
            print("%d: Pacman emerges victorious! Score: %d" % (self.index, state.data.score))
            self.index += 1
        game.gameOver = True

    def lose(self, state, game):
        if not self.quiet:
            print("%d: Pacman died! Score: %d" % (self.index, state.data.score))
            self.index += 1
        game.gameOver = True
...
def readCommand(argv):
    ...
    if options.gameToReplay != None:
        ...
        import pickle
        f = open(options.gameToReplay, 'rb')  # Python 3: binary mode for pickle
        try:
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    for i in range(numGames):
        ...
        if record:
            import pickle
            import time
            fname = ('records/recorded-game-%d' % (i))
            f = open(fname, 'wb')  # Python 3: 'file' -> 'open', binary mode for pickle
