...
def betterEvaluationFunction(currentGameState: GameState):
    ...
    기본: 각 게임의 상태에 따라서 score 값을 계산하여 return 하는 방식. 각 상태에 weight를 부여하여 계산.
    [1] 게임에서 이기거나 지는 상태에는 각각 매우 큰 양수, 음수를 return
    [2] 음식
        [2-1] 가장 가까운 음식과의 거리 (가까울수록 좋음)
        [2-2] 남아있는 음식의 개수 (적을수록 좋음)
    [3] 유령
        [3-1] 겁먹은 유령과의 거리 (가까울수록 좋음)
        [3-2] 겁먹지 않은 유령과의 거리 (멀수록 좋음)
            [3-2-1] 매우 위험한 상태: 일정 거리 이내에 유령이 있는 경우
            [3-2-2] 덜 위험한 상태: 일정 거리 + 2 이내에 유령이 있는 경우
    [4] 캡슐
        [4-1] 가장 가까운 캡슐과의 거리 (가까울수록 좋음)
